...
const QuestHeroItem = ({ hero, itemClickHandler, hitted, healed, reward }: QuestHeroItemProps) => {
  ...
  const psyRef = useRef<HTMLCanvasElement>({} as HTMLCanvasElement);

  // CONTINUE: переделать на блоки без канвасов
  ...
  useEffect(() => {
    ...
    const psyCtx = psyRef.current.getContext('2d')!;
    psyCtx.clearRect(0, 0, canvasW, canvasH);
    psyCtx.fillStyle = 'lightgray';
    psyCtx.fillRect(0, 0, canvasW, canvasH);
    psyCtx.fillStyle = 'ligtblue';
    psyCtx.fillRect(0, 0, canvasW * hero.level.progress, canvasH);
  ...
  return (
    <div className={classes} style={style} onClick={itemClickHandler}>
      ...
      <canvas className='quest-hero-item__psy-bar' ref={psyRef} width={canvasW} height={canvasH} />
